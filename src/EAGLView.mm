...
- (void) scoreReported: (NSError*) error;
{
	...
	if(error == NULL)
	{
     ...
     if (!cppstuff.regularHighscore_button.pressed)
        [self.gameCenterManager reloadHighScoresForCategory: RandomLeaderboardID];
    else 
...
- (void) showLeaderboard;
{
    ...
    if (leaderboardController != nil)
    {
        ...
        if (!cppstuff.regularHighscore_button.pressed)
            leaderboardController.category = RandomLeaderboardID;
        else
...
#ifndef LITE
    if([GameCenterManager isGameCenterAvailable]){
        if (cppstuff.onlineScoreType == 0)
            [self.gameCenterManager reportScore: cppstuff.GameCenterScore forCategory: RegularLeaderboardID];
        else
            [self.gameCenterManager reportScore: cppstuff.GameCenterScore forCategory: RandomLeaderboardID];
    }
#endif
}
//--------------------------------
...
-(void) getScore{
    ...
    NSString *myURL = [[NSString alloc] initWithFormat:@"http://jrs0ul.com/score/BumpyDroids.php?mode=get&type=%d&page=%d",
