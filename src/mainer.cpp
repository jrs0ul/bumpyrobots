...
THREADFUNC getScore(void * args){
    ...
    char url[255];
    sprintf(url, "http://jrs0ul.com/score/BumpyRobotsPC.php?mode=get&page=%d&type=%d", Game.onlineScorePage ,Game.onlineScoreType);
...
int main( int   argc, char *argv[] ){
    ...
    char buf[255];
    GetHomePath(buf);
    sprintf(Game.DocumentPath, "%s.BDroids", buf);
    ...
    if (!SDL.InitWindow("Bumpy Robots 1.23", "icon.bmp")){
        Works = false;
    }
