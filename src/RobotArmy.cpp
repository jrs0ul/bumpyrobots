...
void RobotArmy::create(LevelMap& map){
    ...
    for (unsigned i = 0; i < 10; i++){
        ...
        switch (r.face){
            case PLAYER: r.startFrame =7; break;
            case CRYSTAL : {
                r.startFrame = 2;
                if (crystals > 5){
                    r.face = ASTARER;
...
void RobotArmy::loadFromXMLNode(XmlNode * node){
    ...
    for (unsigned long i = 0; i < node->childrenCount(); i++){
        ...
        switch(r.face){
            case PLAYER   :  r.startFrame = 7; break;
            case CRYSTAL  :  r.startFrame = 2; break;
            case SPIKEY   :  r.startFrame = 1; break;
            case ASTARER  :  r.startFrame = 11; break;
            case ASSAULTER:  r.startFrame = 0; break;
        }
        
        r.frame = (r.face == CRYSTAL)? rand()%2 : 0;
...
void RobotArmy::draw(PicsContainer& pics, int x, int y, float c,
    ...
    if (robots.count()){
        ...
        for (unsigned long i = 1; i < robots.count(); i++){
            if (!robots[i].dead){
                if ((!robots[i].falling)&&(robots[i].face != BOMB))
                    robots[i].drawShadow(pics, x, y, c, screenWidth, screenHeight);
                robots[i].draw(pics, x, y, c, screenWidth, screenHeight);
            }
        }
        if ((!robots[0].dead) && (!robots[0].falling)){
            if (!robots[0].falling)
                robots[0].drawShadow(pics, x, y, c, screenWidth, screenHeight);
            robots[0].draw(pics, x, y, c, screenWidth, screenHeight);
        }
        
        for (int i = (int)((int)robots.count())-1; i >= 0; i--){
            if (!robots[(unsigned long)i].dead)
                robots[(unsigned long)i].particles.drawParticles(pics, 3, Vector3D(x, 0, y));
        }
        
    }
}
//--------------------------------------
bool RobotArmy::isRobotColides(Robot * colider){
    for (unsigned long i = 0; i < robots.count(); i++){
        if (!robots[i].dead){
            Robot * r2 = getRobot(i);
            if (CollisionSphere_Sphere(Vector3D(colider->pos.v[0]+(colider->direction.v[0] * colider->speed),
                                                colider->pos.v[1]+(colider->direction.v[1] * colider->speed),
                                                colider->pos.v[2]+(colider->direction.v[2] * colider->speed)),
                                       colider->radius,
                                       r2->pos,
                                       r2->radius)){
                return true;
...
void RobotArmy::collisionResponse(Robot* r, unsigned long i, float volume){
    ...
    for (int a = 0; a < colidersCount; a++){
        ...
        Vector3D d(r->direction.v[0] * r->speed * 0.5f,
                   r->direction.v[1] * r->speed * 0.5f,
                   r->direction.v[2] * r->speed * 0.5f);
        Vector3D d1(hitter->direction.v[0] * hitter->speed * 0.5f,
                    hitter->direction.v[1] * hitter->speed * 0.5f,
                    hitter->direction.v[2] * hitter->speed * 0.5f);
